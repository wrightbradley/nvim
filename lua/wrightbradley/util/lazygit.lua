...
M.theme_path = Util.norm(vim.fn.stdpath("cache") .. "/lazygit-theme.yml")
  ...
  if vim.g.lazygit_config then
    ...
    if not M.config_dir then
      ...
      if ok then
        ...
        vim.env.LG_CONFIG_FILE = Util.norm(M.config_dir .. "/config.yml" .. "," .. M.theme_path)
        local custom_config = Util.norm(M.config_dir .. "/custom.yml")
...
function M.browse()
  ...
  local function open(remote)
    if remote then
      Util.info(("Opening [%s](%s)"):format(remote.name, remote.url))
  ...
  if #remotes == 0 then
    return Util.error("No git remotes found")
