...
M.sources = {
  { name = "Util", desc = "Util extras", module = "wrightbradley.plugins.extras" },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("wrightbradley.extras")
---@type string[]
M.state = nil
...
function M.wants(opts)
  ...
  if opts.root then
    ...
    return #Util.root.detectors.pattern(M.buf, opts.root) > 0
  end
  return false
end
...
function X.new()
  ...
  self.float = Float.new({ title = "Util Extras" })
    ...
    if extra.row == pos[1] then
      ...
      Util.info(
        "`"
          ...
          .. "\nPlease restart Util to apply the changes.",
        { title = "LazyExtras" }
      )
      self:update()
      return
...
function X:render()
  ...
  self.text
    :append("This is a list of all enabled/disabled Util extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  ...
  if extra.source.name ~= "Util" then
  ...
  for _, import in ipairs(extra.imports) do
    import = import:gsub("^wrightbradley.plugins.extras.", "")
